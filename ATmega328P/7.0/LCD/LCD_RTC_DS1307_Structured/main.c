/*
 * LCD_RTC_DS1307_Structured.c
 *
 * Created: 2017-07-13 오후 8:20:43
 * Author : insoo
 */ 

//-----------------------------------------------------------------------------
/* I2C.c: Experiments with interfacing ATmega328 to an DS1307 RTC

 Target : ATmega328P 
 Created: 2017-07-11 오후 2:27:12
 Author : insoo
 Referece 
...
// ---------------------------------------------------------------------------
// GLOBAL DEFINES
#define F_CPU 1000000L // run CPU at 1 MHz
#define LED PD5 // Boarduino LED on PD5
...
void InitAVR()
{
	...
	DDRD = 0xFC; // 1111.1100; set D2-D7 as outputs
	//DDRC = 0x00; // 0000.0000; set PORTC as inputs
	DDRB = 0xE1; // 1110.0001; set PORTB as out & inputs
}
...
#define LCD_RS PD2  // pin for LCD R/S (eg PB0)
#define LCD_E PD3   // pin for LCD enable
#define DAT4 PD4 // pin for d4
#define DAT5 PD5 // pin for d5
#define DAT6 PD6 // pin for d6
#define DAT7 PD7 // pin for d7
...
void LCD_Init()
{
    _delay_ms(100); // power up delay (insoo added. Jul 13, 2017)
	...
	LCD_Cmd(lcd_FunctionReset);                 // first part of reset sequence
	_delay_ms(10);                                  // 4.1 mS delay (min)

	LCD_Cmd(lcd_FunctionReset);                 // second part of reset sequence
	_delay_us(200);                                 // 100uS delay (min)

	LCD_Cmd(lcd_FunctionReset);                 // third part of reset sequence
