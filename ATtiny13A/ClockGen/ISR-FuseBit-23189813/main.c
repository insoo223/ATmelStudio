/**************************************************************
 ...
 Description: 
	...
	ATtiny13A controls LED blink speed, and its blinking frequency will
		be different even with same code.

 HEX size[Byte]:  122 out of 1024
...
** After burning the fuse as Internal 128Kbps OSC, 
** The communication speed of USBtiny programmer should be low enough matching w/128K MCU
avrdude -B 250 -c usbtiny -P usb -p attiny13 -U flash:w:"ISR-FuseBit.hex":i

** To recover to Internal 9.6Mhz OSC from Internal 128Kbps OSC
avrdude -B 250 -c usbtiny -P usb -p attiny13 -U lfuse:w:0x6A:m flash:w:"ISR-FuseBit.hex":i

** After returning to Internal 9.6Mhz OSC 
...
int main(void)
{
	//*** CAUTION: after prescaling the clock of 128Khz to clock_div_128, 
	//*** USBtiny do not work with any of -B option	
