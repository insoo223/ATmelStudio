...
ISR(WDT_vect) 
{
	...
	//if ( (WDTtick30min >= WAKEUP_PERIOD) || (sysInitStatus == 0 ) )
	if (WDTtick30min >= WAKEUP_PERIOD)
	{
		...
		else if (WDTtick >= SET_DELAY_UNIT)
		{
			...
			//After initial power-on measurement is done, set the system init variable as 1
			// so that prevent data log two times in a row between every one hour sleep
			// and the next WDT tick
			//eeprom_update_byte((uint8_t*)SYSINIT_ADDR, 1);
			//sysInitStatus = 1;
		}//else if (WDTtick >= SET_DELAY_UNIT)
	}
...
int main(void) {
	...
	_delay_ms(10000);
	PORTB = 0<<NPN_TR_PORT; //turn on MOSFET
