...
Select Tool - USBtiny (USBtiny menu should be configured in the external tool menu)

<For CMD window or DOS prompt>
cd "C:\Users\insoo\Documents\GitHub\ATmelStudio\ATtiny13A\ClockGen\TMRCNT-Sound-23188757\Debug"
...
void playOctaveNote(uint8_t oct, int adc)
{	
	if (adc < 128)
		tone(oct,0);
	else if (adc < 256)
...
void adc_setup (void)
{
	// Set the ADC input to PB5/ADC0, left adjust result
	//ADMUX |= (1 << MUX0) | (1 << ADLAR);
	// Set the ADC input to PB2/ADC1, right adjusted (default)
	ADMUX |= (1 << MUX1) ; 
	// Set the ADC input to PB4/ADC2, right adjusted (default)
	//ADMUX |= (1 << MUX2) ;
	// Set the ADC input to PB3/ADC3, right adjusted (default)
	//ADMUX |= (1 << MUX3) ;
	...
	ADCSRA |= (1 << ADPS0); // added by insoo (2018.9.11)
	ADCSRA |= (1 << ADPS1); // added by insoo (2018.9.11)
	ADCSRA |= (1 << ADPS2); // added by insoo (2018.9.11)
